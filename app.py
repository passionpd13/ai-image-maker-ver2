...
def generate_prompt(api_key, index, text_chunk, style_instruction, video_title, genre_mode="info", target_language="Korean"):
    ...
    - **한글**로만 출력하십시오.
        ...
        # [UPDATED] 극장판 애니메이션 모드 (정보성 배제, 시네마틱 강조)
        full_instruction = f"""
    [역할]
    당신은 '너의 이름은', '바이올렛 에버가든', '귀멸의 칼날'과 같은 **최정상급 일본 극장판 애니메이션의 '총괄 감독'**입니다.
    대본 내용을 설명하거나 요약하는 '삽화'를 그리는 것이 **아닙니다.**
    대본 속 상황이 실제로 벌어지고 있는 **'결정적인 애니메이션 장면(Anime Screenshot)'**을 시각화해야 합니다.

    [전체 영상 주제] "{video_title}"
    [스타일 가이드] {style_instruction}

    [🚫 절대 금지 사항 (Negative Constraints)]
    1. **인포그래픽 스타일 금지:** 화면에 설명 화살표, 그래프, 아이콘이 둥둥 떠다니게 하지 마십시오.
    2. **요약 금지:** 대본을 설명하려 들지 말고, 대본 속 **'공간'과 '인물'의 감정**을 보여주십시오.
    3. **분할 화면 금지:** 만화책 컷 분할(Comics panel)이나 화면 쪼개기를 하지 마십시오. 오직 **하나의 시네마틱한 샷(Single Cinematic Shot)**입니다.

    [필수 연출 지침]
    1. **작화 스타일 (Cinematic Anime):**
       - 셀 애니메이션 특유의 **감성적인 빛과 그림자(Ray Tracing, Tyndall Effect)**를 강조하십시오.
       - 배경은 사진처럼 정밀하되, 애니메이션 특유의 보정(Color Grading)이 들어간 느낌이어야 합니다.
    2. **카메라 워킹 (Camera Angles):**
       - 평범한 정면 샷(Front shot)을 피하십시오.
       - **로우 앵글(Low Angle), 하이 앵글(High Angle), 광각(Wide Lens), 클로즈업(Close-up)** 등을 활용하여 영화적인 긴장감을 주십시오.
       - 피사계 심도(Depth of Field, Bokeh)를 활용하여 인물을 강조하십시오.
    3. **캐릭터 연기 (Character Acting):**
       - 캐릭터는 전형적인 '일본 미소년/미소녀(Bishonen/Bishoujo)' 스타일 혹은 대본에 맞는 디자인입니다.
       - **눈빛(Eye contact)**과 **표정의 디테일(눈물 고임, 홍조, 미간 찌푸림 등)**을 통해 대본의 감정을 극대화하십시오.
    4. **텍스트 처리 (Diegetic Text Only):** {lang_guide}
       - **중요:** 자막처럼 허공에 글자를 쓰지 마십시오.
       - 텍스트가 필요하다면 **가게 간판, 들고 있는 스마트폰 화면, 책 표지, 칠판의 낙서**처럼 **'물건'의 일부**로 자연스럽게 배치하십시오.

    [임무]
    대본을 **영화 시나리오**로 해석하여, AI가 그릴 수 있는 **최고 퀄리티의 애니메이션 프롬프트**를 작성하십시오.
    - 묘사는 구체적이고 감각적이어야 합니다. (예: "석양빛이 쏟아지는 교실 창가에 기대어...", "비에 젖은 아스팔트 위를 달리는...")
    - **한글**로만 출력하십시오.
        """
