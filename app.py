...
def generate_prompt(api_key, index, text_chunk, style_instruction, video_title, genre_mode="info", target_language="Korean"):
    ...
    - **한글**로만 출력하십시오.
        ...
        # [UPDATED] 정보 전달 + 고퀄리티 애니메이션 (상황극/컨셉 아트)
        full_instruction = f"""
    [역할]
    당신은 100만 유튜버의 영상을 책임지는 **'일본 애니메이션 비주얼 스토리텔러'**입니다.
    당신의 목표는 두 가지를 동시에 달성하는 것입니다.
    1. **정보 전달(Information):** 시청자가 그림만 봐도 "아, 지금 무슨 이야기를 하는구나"라고 직관적으로 알아야 합니다.
    2. **시각적 재미(Visual Fun):** 지루한 인포그래픽 대신, **'일본 애니메이션 캐릭터의 연기와 상황'**으로 보여줘야 합니다.
    ...
    [필수 연출 지침 - 밸런스 조절]
    1. **서정성 금지, 직관성 강조:**
       - 단순히 비 내리는 창가나 예쁜 풍경만 그리지 마십시오. (내용 전달 안 됨)
       - 대본의 핵심 소재(돈, 경쟁, 위험, 미래 등)가 시각적으로 **'등장'**해야 합니다.
    2. **추상적 개념의 '구체화(Reification)':**
       - 대본이 '경제 위기'라면 -> 거대한 붉은 화살표가 땅에 꽂히고, 그 앞에서 방어 자세를 취하는 애니메이션 전사 캐릭터.
       - 대본이 '경쟁 심화'라면 -> 좁은 문을 통과하기 위해 서로 밀치며 달리는 양복 입은 캐릭터들의 역동적 구도.
    3. **캐릭터 연기(Acting):**
       - 캐릭터는 예쁘기만 한 게 아니라, 상황에 맞는 **'리액션'**을 해야 합니다.
       - 당황, 환호, 의심, 공포 등 대본의 뉘앙스를 **과장된 애니메이션 표정**으로 연기하십시오.
    4. **작화 스타일:**
       - 선명하고 쨍한 **'최신 TV 애니메이션(TVA) 오프닝'** 느낌으로 그리십시오.
       - 흐릿하거나 몽환적인 예술 영화 느낌보다는, **정보량이 많고 선명한(High Contrast & Saturation)** 스타일입니다.
    5. **텍스트 활용:** {lang_guide}
       - 정보 전달을 위해 **핵심 키워드 1~2단어**는 만화적인 효과음이나 배경 타이포그래피처럼 **임팩트 있게 배치**해도 좋습니다. (지루한 자막 아님)

    [임무]
    대본의 핵심 메시지를 **'한 컷의 애니메이션 일러스트'**로 시각화하는 프롬프트를 작성하십시오.
    - **핵심:** "무엇(Subject)이 어떠한 행동(Action)을 하고 있고, 주변에 어떤 상징물(Symbol)이 있는가"를 명확히 하십시오.
    - **한글**로만 출력하십시오.
        """
