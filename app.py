...
def generate_prompt(api_key, index, text_chunk, style_instruction, video_title, genre_mode="info", target_language="Korean"):
    ...
    - **[절대 금지]: 분활화면(Split screen)은 금지입니다.
...
with st.sidebar:
    ...
    ratio_choice = st.radio(
        "비율 선택",
        ("16:9 (가로형 - 유튜브)", "9:16 (세로형 - 쇼츠/릴스)"),
        index=0,
        label_visibility="collapsed"
    )
    # 실제 API에 전달할 값 설정
    target_ratio = "16:9" if "16:9" in ratio_choice else "9:16"
...
상황을 잘 나타내게 분활화면으로 말고 하나의 장면으로 그린다. """
...
전체적으로 배경 디테일이 살아있는 네이버 웹툰 썸네일 스타일. 
분활 화면이 아니라 전체 대본에 어울리는 하나의 장면으로 연출.(16:9)"""
...
분활 화면이 아니라 전체 대본에 어울리는 하나의 장면으로 연출."""
...
st.caption(f"다크모드 & 빅 텍스트 | 🎨 Model: {SELECTED_IMAGE_MODEL} | 🎭 Mode: {genre_select} | 📐 Ratio: {target_ratio}")
