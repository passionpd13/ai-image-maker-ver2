...
def generate_prompt(api_key, index, text_chunk, style_instruction, video_title, genre_mode="info", target_language="Korean", character_desc="", target_layout="16:9 와이드 비율"):
    ...
    **얼굴 표정이 단순한 스틱맨이므로, '몸짓(Body Language)'으로 감정을 표현하는 데 집중하십시오.**
    ...
    # ---------------------------------------------------------
    # [모드 5] History/Satire Stickman (History Matters Style) - [수정됨: 담백한 단일 초점 연출]
    # ---------------------------------------------------------
    elif genre_mode == "paint_explainer":
        full_instruction = f"""
    {common_header}
    [역할]
    당신은 'History Matters' 또는 'OverSimplified' 스타일의 **'초심플 미니멀리즘 애니메이션 감독'**입니다.
    화면이 복잡해지는 것을 극도로 싫어합니다. **'한 장면에는 오직 하나의 주제(One Subject)'**만 등장시키는 것을 원칙으로 합니다.

    [전체 영상 주제] "{video_title}"
    [스타일 가이드] {style_instruction}

    [🚫 절대 금지 (Negative Constraint)]
    - **복잡한 합성 금지:** 지도 위에 사람을 세우고, 옆에 건물을 그리고, 하늘에 비행기를 띄우는 식의 **'종합 선물 세트' 같은 복잡한 구도를 절대 금지**합니다.
    - **배경 묘사 금지:** 배경은 무조건 **단색(Solid Color)**이거나 **빈 공간**이어야 합니다.

    [핵심 비주얼 스타일 가이드]
    1. **[단일 초점 원칙 (Single Focus Principle)]:**
        대본의 핵심 단어 하나를 골라 그것만 **'크고 단순하게'** 그리십시오.
        - **인물 중심일 때:** 배경이나 건물을 다 없애고, **오직 캐릭터 1~2명의 표정과 연기**에만 집중하십시오. (지도나 건물 그리지 마세요)
        - **장소 중심일 때:** 사람을 빼고, **오직 '건물(Building)' 하나만** 화면 중앙에 덩그러니 그리십시오. (예: 은행 건물이면 'BANK' 간판 달린 건물 하나만)
        - **사물 중심일 때:** 사람 없이 **'물건(돈, 계약서, 무기)'**만 클로즈업해서 보여주십시오.
        - **섞어야 할 때:** 꼭 필요한 경우에만 섞되, 최대한 단순하게 표현하십시오. (예: 사람이 사물을 들고 있는 정도)

    2. **[작화 스타일]:**
        - **History Matters 스타일:** 매우 굵은 검은색 외곽선 (5px 이상), 명암 없는 완전한 플랫 컬러.
        - **캐릭터:** 하얀 동그라미 얼굴, 점 눈, 재치 있는 표정(다크서클, 땀방울).

    3. **[텍스트 - 굵은 손글씨]:**
        - 사람이 마카펜으로 쓴 듯한 **'투박한 손글씨'**로, 핵심 키워드(라벨)만 사물 옆에 붙이십시오. ({lang_guide})

    4. **[구도]:** 분할 화면 금지. **{target_layout}**.
        - 피사체를 화면 중앙에 크게 배치하여 여백을 살리십시오.

    [9:16 세로 모드 지침]
    - 인물이면 상반신 클로즈업, 건물이면 건물 전체가 꽉 차게, 사물이면 화면 가득 차게.

    [임무]
    대본을 보고 **'가장 중요한 단 하나'**만 남기고 나머지는 다 버리는 **'담백한 일러스트 프롬프트'**를 작성하십시오.
    - **필수 키워드:** "History Matters style, OverSimplified style, minimalism, thick black outlines, flat colors, one subject focus, solid background, minimal elements"
    - **한글**로만 출력하십시오.
    - (지문) 같은 부연설명 연출 지시어는 제외한다.
        """

    # ---------------------------------------------------------
    # [모드 6] 심플 그림판 (스토리형/배경강화) - [수정됨: 배경 디테일업 + 간판/사물 텍스트 허용]
    # ---------------------------------------------------------
    ...
    **[배경 및 공간 연출 강화 (Enhanced Minimal Stage Setting)]:**
    ...
    2. **[텍스트 제한적 허용 - 사물 내 자연스러운 연출 (Limited Diegetic Text)]:**
        ...
        - **언어 엄수:** {lang_guide} (사물에 적힌 텍스트도 이 언어 규칙을 따라야 합니다.)
...
텍스트: 굵은 마카펜 손글씨 느낌. 꼭 필요한 라벨링만 최소화하여 배치.
...
핵심 원칙: **"한 장면에 오직 하나의 주제만(Single Focus)"** 담백하게 연출. 복잡한 합성 금지.
핵심: "텍스트 없는 시각적 스토리텔링".
배경: 기본 하얀색 베이스에 상황을 설명하는 **'단순화된 무대 세트(Simple Stage Set)'** 구성 (예: 감옥 벽과 창살, 사무실 책상과 캐비닛).(예: 감옥 -> 창살, 사무실 -> 책상, 전쟁 -> 참호).
